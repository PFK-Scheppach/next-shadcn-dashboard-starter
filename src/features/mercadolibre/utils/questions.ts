...
export async function replyToQuestion(questionId: number, answer: string): Promise<boolean> {
  try {
...
export function getQuestionStatusColor(question: QuestionWithStatus): string {
  ...
  const hoursSinceAsked = (new Date().getTime() - new Date(question.date_created).getTime()) / (1000 * 60 * 60);
